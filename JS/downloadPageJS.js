let Images = Array.from(
    document.querySelectorAll(".container .imgs-content .img-box")
);
Images.forEach((box) => {
    box.addEventListener("click", (ev) => {
        ...
        box.innerHTML = `<img id="logo" src="Images/${
            ev.currentTarget.querySelector(".photo").dataset.from
        }-logo.avif" />`;
        let downloadBox = document.createElement("div");
        downloadBox.classList.add("download-box");
        downloadBox.innerHTML = `<a download href="Images/download/${
            ev.currentTarget.querySelector(".photo").alt
        }.jpg">Download <i class="pe-7s-download"></i></a>`;
	...
	<p>Type: JPG`;
        ...
        popupBox.appendChild(
            ev.currentTarget.querySelector(".photo").cloneNode()
        );
